...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    **The Answer:** A streak length of 1 means that there was a hit "H" followed by a miss "M". 
    and the streak length of 0 means there is a miss "M" followed by a miss "M".
...
1.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    **The Answer:** it shows the typical streak lenghth is 0, and longest sreak length is 4, the distripution is right skewed and unimodal with 0-4 range, where the median and mode are 0 length.
...
1.  In your simulation of flipping the unfair coin 100 times, how many flips 
     ...
     **The Answer:** I got 18 heads, and 82 tails.
     
```{r}
#setting the seed
set.seed(01011989)
#assuming the probability 0.2 heads, and 0.8 tails
...
1.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    **The Answer:** change the probabilty range to 0.45 - 0.55
...
1.  Using `calc_streak`, compute the streak lengths of `sim_basket`, and
    save the results in a data frame called `sim_basket`.
...
1.  Describe the distribution of streak lengths. What is the typical streak 
    ...
    **The answer:** the length is 0 - 6, andthe typical sreak length is 0.
...
1.  If you were to run the simulation of the independent shooter a second time, 
    ...
    **The answer:** the distripution is little different but within the range of probability which is around 45. the distripution is the same which is right skewed.
...
1.  How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    **The answer:** it doesn't seem like the shooter has a hot hand as Kobe in 2009.

```{r}
#Kobe streaks
ggplot(data = kobe_streak, aes(x = length)) + ggtitle ("Kobe shooter")+ geom_bar()

#shooter streaks
ggplot(data = sim_basket, aes(x = length)) + ggtitle ("Independent shooter")+ geom_bar()
```
